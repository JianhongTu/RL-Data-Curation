...
# Optional wandb import (only needed if not in debug mode)
try:
    import wandb
...
from sb3_comparison.openhermes.embeddings import build_openhermes_embeddings
...
def main(debug: bool = False):
    print("=" * 60)
    print("Starting OpenHermes 2.5 PPO Training")
    print("=" * 60)
    
    print("\n[1/6] Loading OpenHermes 2.5 embeddings...")
    X_unit, y = build_openhermes_embeddings()
    ...
    if max_path.exists() and min_path.exists():
        ...
        # W&B run (skip in fast debug mode or if wandb not available)
        run = None
    ...
    print("\n[6/6] Generating plot...")
    plot_results(size_percents, results_max, results_min, results_rand, "openhermes_plot.png", results_max_ppov1, results_min_ppov1)
    print(f"✓ Plot saved to openhermes_plot.png")
    
    print("\n" + "=" * 60)
    print("✓ OpenHermes 2.5 PPO Training Complete!")
    print("=" * 60)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="OpenHermes 2.5 PPO comparison")
    parser.add_argument(
        "--fast-debug",
        action="store_true",
